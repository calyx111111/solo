...
public class AgentProcessManager implements Disposable {
    ...
    private final Object tsBackendExtractLock = new Object();
        ...
        try {
            ...
            tsProcessHandler.addProcessListener(new ProcessAdapter() {
                ...
                public void processTerminated(@NotNull ProcessEvent event) {
                    ...
                    LOG.warn("[TS Agent] exit code " + code);
    ...
    private Path extractTypeScriptBackend() {
        String backendHash = readBundledBackendHash();
        if (backendHash == null || backendHash.isBlank()) {
            LOG.warn("Missing /ts-backend/.backend.hash, cannot determine backend content hash.");
            return null;
        }
        LOG.info("Get backend hash: " + backendHash);

        Path backendRoot = Paths.get(PathManager.getSystemPath(), "solo_mode", "ts-backend");
        Path targetDir = backendRoot.resolve(backendHash);
        Path entryPoint = targetDir.resolve("dist").resolve("index.js");
        Path completeMarker = targetDir.resolve(".extract-complete");

        try {
            Files.createDirectories(backendRoot);
        } catch (IOException e) {
            LOG.warn("Failed to create backend root dir: " + backendRoot + ", " + e.getMessage(), e);
            return null;
        }
        ...
        synchronized (tsBackendExtractLock) {
            ...
            try {
                // 如果当前 hash 目录存在但不完整，先删掉重解压
                if (Files.exists(targetDir)) {
                    LOG.warn("Backend dir exists but is incomplete, deleting: " + targetDir);
                    deleteDirectory(targetDir);
                }

                // 仅当 hash 不一致时，删除旧 hash 目录
                deleteStaleBackendDirs(backendRoot, backendHash);

                Files.createDirectories(targetDir);

                extractResourceDirectory("/ts-backend", targetDir);

                if (!Files.isRegularFile(entryPoint)) {
                    throw new IOException("Extraction finished but dist/index.js not found: " + entryPoint);
                }
                ...
                LOG.info("TypeScript backend extracted to: " + targetDir + ", hash=" + backendHash);
                return targetDir;
            } catch (IOException e) {
                LOG.warn("Failed to extract TypeScript backend, hash=" + backendHash + ": " + e.getMessage(), e);
                if (Files.exists(targetDir)) {
                    deleteDirectory(targetDir);
                }
                return null;
    ...
    private String readBundledBackendHash() {
        try (InputStream in = getClass().getResourceAsStream("/ts-backend/.backend.hash")) {
            if (in == null) {
                return null;
            }
            return new String(in.readAllBytes(), StandardCharsets.UTF_8).trim();
        } catch (IOException e) {
            LOG.warn("Failed to read /ts-backend/.backend.hash: " + e.getMessage(), e);
            return null;
        }
    }
    ...
    private void deleteStaleBackendDirs(Path backendRoot, String currentHash) throws IOException {
        if (!Files.isDirectory(backendRoot)) {
            return;
        }

        try (Stream<Path> stream = Files.list(backendRoot)) {
            stream.filter(Files::isDirectory)
                    .filter(path -> !Objects.equals(path.getFileName().toString(), currentHash))
                    .forEach(path -> {
                        LOG.info("Deleting stale TypeScript backend dir: " + path);
