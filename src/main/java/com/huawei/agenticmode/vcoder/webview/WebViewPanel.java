...
public class WebViewPanel implements Disposable {
    ...
    private void setupLoadHandler() {
        ...
        // Setup theme listener to detect IntelliJ theme changes
        setupThemeListener();
        ...
        browser.getJBCefClient().addDisplayHandler(new CefDisplayHandlerAdapter() {
            ...
            public boolean onCursorChange(CefBrowser browser, int cursorType) {
                if (browser.getUIComponent().getCursor().getType() == cursorType) {
                    return false;
                }
                browser.getUIComponent().setCursor(java.awt.Cursor.getPredefinedCursor(cursorType));
                return false;
            }
        }, browser.getCefBrowser());
    ...
    private String getCurrentLanguage() {
        try {
            ...
            LOG.warn("Failed to get current language", e);
            return Locale.getDefault().getLanguage();
        }
    }
            ...
            public void lookAndFeelChanged(LafManager source) {
                if (browser != null) {
                    ...
                    String themeScript = String.format("""
                            window.__vcoderEventBus && window.__vcoderEventBus.emit('intellij-theme-changed', { isDarkTheme: %b, panelBackground: '%s' });
                            console.log('[VCoder] IntelliJ theme changed:', %b);
                            """, isDarkTheme, colorToHex(panelBackground), isDarkTheme);
                    browser.getCefBrowser().executeJavaScript(themeScript, "", 0);
                }
            }
        });
    }
    ...
    private boolean hasWebviewResource() {
        try {
            var plugin = PluginManager.getInstance().findEnabledPlugin(PluginId.getId("com.huawei.agenticmode"));
